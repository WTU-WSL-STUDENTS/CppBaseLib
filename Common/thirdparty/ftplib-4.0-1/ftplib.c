 ...
 */
GLOBALDEF int FtpConnect(const char *host, netbuf **nControl, int timeout/* = 0 */)
{
    int sControl;
	...
	*pnum++ = '\0';
	...
	if(0 < timeout)
	{
		setsockopt(sControl, SOL_SOCKET, SO_SNDTIMEO, (char *)&timeout, sizeof(timeout));
		setsockopt(sControl, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout,sizeof(timeout));
	}
